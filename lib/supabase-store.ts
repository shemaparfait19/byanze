...
interface SupabaseStore {
  ...
  addInvoice: (invoice: Omit<Invoice, 'createdAt' | 'updatedAt'>) => Promise<void>
  ...
  updateInvoiceStatus: (id: string, status: 'pending' | 'completed' | 'cancelled') => Promise<void>
}
...
export const useSupabaseStore = create<SupabaseStore>((set, get) => ({
  ...
  checkDatabaseSetup: async () => {
    try {
      ...
      set({ databaseReady: false, error: `Database setup required: ${error.message}` })
      return false
    }
  },
  ...
  initializeDatabase: async () => {
    try {
      ...
      if (isReady) {
        ...
        set({ 
          error: 'Database tables not found. Please run the setup script in Supabase SQL Editor.',
          loading: false,
          databaseReady: false
        })
      }
  ...
  updateClient: async (id, updates) => {
    try {
      ...
      if (updates.rewardClaimed !== undefined) updateData.reward_claimed = updates.rewardClaimed
  ...
  updateInvoice: async (id, updates) => {
    try {
      ...
      if (updates.pickupDate !== undefined) updateData.pickup_date = updates.pickupDate || null
      if (updates.pickupTime !== undefined) updateData.pickup_time = updates.pickupTime || null
  ...
  getPickupNotifications: () => {
    ...
    return get().invoices.filter(invoice => {
      if (!invoice.pickupDate || !invoice.pickupTime || invoice.status === 'completed') {
        return false
      }
